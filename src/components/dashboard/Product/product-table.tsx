...
import { iProduct } from "@/models/StoreAdmin/product.model"
...
export function ProductsTable({
    ...
    const handleDelete = async (id: string, name?: string, skipConfirm?: boolean) => {
        if (!skipConfirm) {
            const confirmed = confirm(`Are you sure you want to delete the product "${name}"?`)
            if (!confirmed) return
        }

        try {
            await deleteProduct(id)
            if (!skipConfirm) toast.success("Product deleted successfully")
        } catch (error: any) {
    ...
    const handleChangeStatus = async (id: string, status: boolean, skipConfirm?: boolean) => {
        if (!skipConfirm) {
            const confirmed = confirm(`Are you sure you want to change the product status?`)
            if (!confirmed) return
        }

        try {
            await togglePoductStatus(id, status)
            if (!skipConfirm) toast.success("Product status changed successfully")
        } catch (error: any) {
    ...
    const handleMultipleStatus = async (status: boolean) => {
        ...
        try {
            ...
            toast.error(error.message || "Failed to change products status")
        }
    }
