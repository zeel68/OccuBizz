...
import { iProduct, iProductFilters } from "@/models/StoreAdmin/product.model"
...
export default function ProductsPage() {
    ...
    useEffect(() => {
        ...
        // Add filters only if they're not the default value
        if (appliedFilters.search) queryParams.search = appliedFilters.search
        if (appliedFilters.category !== "all") queryParams.category = appliedFilters.category
        if (appliedFilters.parent_category !== "all") queryParams.parent_category = appliedFilters.parent_category
        if (appliedFilters.status !== "all") queryParams.status = appliedFilters.status
        if (appliedFilters.stock_level !== "all") queryParams.stock_level = appliedFilters.stock_level
        if (appliedFilters.price_min) queryParams.price_min = appliedFilters.price_min
        if (appliedFilters.price_max) queryParams.price_max = appliedFilters.price_max
            ...
            {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{stats.avg_rating?.toFixed(1) || 0}</div>
                            <p className="text-xs text-muted-foreground">
