    ...
    const renderCategoryRow = (category: Category, level = 0) => {
        ...
        return (
            <React.Fragment key={category._id}>
                <TableRow className={`${isSelected ? "bg-muted/50" : ""} ${level > 0 ? "bg-muted/20" : ""}`}>
                    <TableCell>
                        <input
                            type="checkbox"
                            checked={isSelected}
                    ...
                    <TableCell>
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                                ...
                                <DropdownMenuItem
                                    onClick={() => handleDelete(category._id, category.display_name)}
...
export default function CategoriesPage() {
    ...
    const stats = useMemo(
        () => ({
            ...
            primary: categories.filter((cat) => cat.is_primary || !cat.parent_id).length,
            inactive: categories.filter((cat) => cat.is_active === false).length,
            subcategories: categories.filter((cat) => !cat.is_primary && cat.parent_id).length,
        }),
        [categories]
    );
    ...
    return (
        <div className="space-y-6 bg-background p-6">
            ...
            {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card className="hover:shadow-md transition-shadow duration-200">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Primary Categories</CardTitle>
                        <Tag className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{stats.primary}</div>
                        <p className="text-xs text-muted-foreground">Main category groups</p>
                    </CardContent>
                </Card>
                        ...
                        <div className="mt-4 p-4 bg-muted/30 rounded-lg border space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    >
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="primary">Primary Categories</SelectItem>
                            ...
                            {hasActiveFilters && (
                                <div className="flex flex-wrap items-center gap-2 pt-2 border-t">
                                    ...
                                    {filterType !== "all" && (
                                        <Badge variant="secondary" className="flex items-center gap-1" onClick={() => setFilterType("all")}>
                                            Type: {filterType}
                                            <X
                                                className="h-3 w-3 cursor-pointer hover:text-destructive"
                                    ...
                                    {sortType !== "name-asc" && (
                                        <Badge variant="secondary" className="flex items-center gap-1" onClick={() => setSortType("name-asc")}>
